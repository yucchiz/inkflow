...
> 各プロパティは `(for colorScheme: ColorScheme)` 引数を取る。`@Environment(\.colorScheme)` で取得した値を渡す。

| プロパティ | ライト | ダーク |
|-----------|--------|--------|
| `Color.InkFlow.bg(for:)` | `#E8EDF5` | `#0C1525` |
| `Color.InkFlow.bgSub(for:)` | `#F0F3F9` | `#162038` |
| `Color.InkFlow.text(for:)` | `#1A2340` | `#E2E8F4` |
| `Color.InkFlow.textSub(for:)` | `#5C6785` | `#8A95B0` |
| `Color.InkFlow.border(for:)` | `#C8D0E0` | `#253050` |
| `Color.InkFlow.accent(for:)` | `#1E40AF` | `#2D6CD6` |
| `Color.InkFlow.accentHover(for:)` | `#1E3A8A` | `#3570D4` |
| `Color.InkFlow.danger(for:)` | `#B91C1C` | `#F45252` |
| `Color.InkFlow.dangerBg(for:)` | `#B91C1C` | `#DC2626` |
...
```swift
// 本文（Noto Serif JP バンドル予定、現在はシステムセリフ）
Typography.body()  // .system(.body, design: .serif)
// UI
Typography.ui()    // .system(.body)
```

Noto Serif JP 400 をバンドル予定。現在はシステムのセリフフォントにフォールバック。UI はシステムフォント（San Francisco）。Dynamic Type に自動対応。
